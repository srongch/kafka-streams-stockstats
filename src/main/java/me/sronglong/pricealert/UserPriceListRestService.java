...
import me.sronglong.pricealert.model.PriceListBean;
...
@Path("state")
public class UserPriceListRestService {
  ...
  private static final Logger log = LoggerFactory.getLogger(UserPriceListRestService.class);

  public UserPriceListRestService(final KafkaStreams streams,
  ...
  public PriceListBean byKey(@PathParam("storeName") final String storeName,
                                 @PathParam("key") final String key) {
    System.out.println("loook up for storename : " + storeName + " and key :" + key);
    ...
    final ReadOnlyKeyValueStore<String, UserPriceSettingStats> store = streams.store(storeName, QueryableStoreTypes.keyValueStore());
    ...
    final UserPriceSettingStats value = store.get(key);
    if (value == null) {
//      throw new NotFoundException();
      return new PriceListBean();
    }
    System.out.println("found : " +value.toString());
    return new PriceListBean(value);
  }

  private PriceListBean fetchByKey(final HostStoreInfo host, final String path) {
    return client.target(String.format("http://%s:%d/%s", host.getHost(), host.getPort(), path))
        .request(MediaType.APPLICATION_JSON_TYPE)
        .get(new GenericType<PriceListBean>() {
        });
  }
